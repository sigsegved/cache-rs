//! Configuration for the concurrent LFUDA cache.

extern crate std;

use super::LfudaCacheConfig;
use core::fmt;
...
/// Configuration for a concurrent LFUDA cache with segmented storage.
///
/// This is the **only** way to configure and create a concurrent LFUDA cache.
...
impl ConcurrentLfudaCacheConfig {
    /// Creates a new concurrent LFUDA cache configuration with the specified capacity.
    ...
    pub fn new(capacity: NonZeroUsize) -> Self {
        ...
        Self {
            base: LfudaCacheConfig::new(capacity),
            segments,
        }
    }
    ...
    #[inline]
    pub fn base_config(&self) -> &LfudaCacheConfig {
        &self.base
    }
}

impl fmt::Debug for ConcurrentLfudaCacheConfig {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("ConcurrentLfudaCacheConfig")
...
mod tests {
    ...
    #[test]
    fn test_concurrent_lfuda_config_creation() {
        let config = ConcurrentLfudaCacheConfig::new(NonZeroUsize::new(1000).unwrap());
    ...
    #[test]
    fn test_concurrent_lfuda_config_builder() {
        let config = ConcurrentLfudaCacheConfig::new(NonZeroUsize::new(1000).unwrap())
            .with_segments(16)
        ...
        assert_eq!(config.initial_age(), 100);
    }
}
