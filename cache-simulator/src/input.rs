...
impl LogReader {
    ...
    fn parse_line(line: &str, line_num: usize) -> Result<Option<Request>, LogParseError> {
        ...
        if line.trim().is_empty()
            || line.starts_with('#')
            || (line_num == 0 && line.contains("timestamp"))
        {
            return Ok(None);
        }

        // Parse the line (format: timestamp, key, size, ttl)
        let parts: Vec<&str> = line.split(',').map(|s| s.trim()).collect();
        if parts.len() < 3 {
            return Err(LogParseError::ParseError(format!(
                "Line {} has too few fields (expected at least 3, got {})",
                line_num + 1,
                parts.len()
            )));
        }
        ...
        let timestamp = match parts[0].parse::<u64>() {
            Ok(ts) => UNIX_EPOCH + Duration::from_secs(ts),
            Err(_) => {
                return Err(LogParseError::ParseError(format!(
